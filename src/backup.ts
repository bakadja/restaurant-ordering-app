...
function onAdd(event: Event) {
  ...
  if (dataId) {
   // const data: MenuProps[] = ;
    ...
    const filteredData = menuArray.filter(menu => menu.id === dataId)
    const addItem = getAddItem(filteredData)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const listItems = getListItems(menuArray)
  //const menuItem = document.getElementById("menu-item")!
  render(listItems, orderMenuEl);
});
